...
### 4. リモート管理機能 (Linux CUI サーバー / 手元PCからの遠隔操作)
本アプリは **「リモート操作API（ホスト機能）」** と **「遠隔操作コントローラー（クライアント機能）」** の両方を内蔵しています。

#### A. 手元PCからリモートホストに接続する場合（クライアント機能）
1. アプリ上部ヘッダーの **「🌐 リモート接続」** ボタンをクリックします。
2. 接続先ホスト（例: `192.168.1.50:18767`）とパスワード（設定されている場合）を入力し、**「接続」** をクリックします。
3. 接続すると、使い慣れた画面のままホスト側の放送タイトル、再生中動画、ストック、リクエスト、コメントが手元画面にリアルタイム同期され、手元から直接操作できます。
4. 手元PCでログインしていれば、接続時に **ニコニコのログインセッションがホスト側へ自動転送** されます。

#### B. Linux CUI環境でホストとして起動する場合
ディスプレイのない Linux サーバーや Docker 上で 24時間起動し、手元のPCから遠隔操作できます。

- **Docker Compose を使用する場合（推奨）**:
  ```bash
  docker compose up -d
  ```
  設定やセッション Cookie は `./data` ディレクトリに永続化されます。

- **直接 Linux コマンドラインから起動する場合**:
  ```bash
  cd app
  npm install
  # Xvfb 仮想ディスプレイ経由でヘッドレス起動
  xvfb-run -a npm run start:headless
  ```
  環境変数（`STSEN_HOST_PORT=18767`, `STSEN_HOST_PASSWORD=...`）や引数（`--port`, `--password`）で設定可能です。
