...
### アプリ内蔵ブラウザによる直接ログイン
- アプリ画面右上の「🔑 ログイン」ボタンをクリックすると、公式のログイン画面が内蔵ブラウザで開きます。
- 通常通りログイン（ReCaptcha / 2段階認証 / SNS認証対応）を完了すると、自動的にログインウィンドウが閉じ、セッション情報がローカル保存されます。
- 次回以降はログイン操作不要で、アプリ起動時に自動的にログイン状態が維持されます。

### 配信枠への接続方法
1. **ブラウザ拡張機能経由（自動）**: ブラウザでニコ生視聴・配信ページを開くだけで自動接続されます。
2. **アプリ内手動入力（単体動作）**: アプリ上部の「lv...」入力欄に配信ID（例: lv351349046）を入力して「接続」を押すと、ブラウザ拡張機能を使わずにアプリ単体で枠に接続できます。
...
リポジトリにタグをプッシュするか、GitHub の Actions タブから手動実行することで、Windows 向けインストーラ（.exe）およびポータブル版（.zip）が自動ビルドされ、GitHub Releases に公開されます。
...
1. **タグをプッシュしてリリースする場合**:
   ...
   \\n2. **GitHub の Web 画面から手動でリリースする場合**:
   - GitHub リポジトリの **Actions** タブを開きます。
   - 左側の **Build & Release** ワークフローを選択します。
   - **Run workflow** ボタンをクリックし、タグ名を入力して実行します。
